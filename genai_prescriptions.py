...
PLAYBOOK_SYSTEM = (
    ...
    "Given short, structured XAI findings from a DGA detection model, "
    "output a concise, prescriptive response plan."
)

PLAYBOOK_USER_TMPL = (
    "Create a prescriptive incident response playbook for a suspected DGA domain.\n\n"
    "Constraints:\n"
    "- Keep to 6–10 concrete steps grouped under phases: "
    "Immediate Containment, Investigation, Eradication/Recovery, and Follow-Up.\n"
...
def generate_playbook(xai_findings: str, api_key: str | None = None) -> str:
    api_key = api_key or os.environ.get("GOOGLE_API_KEY")
    if not api_key:
        return (
            "[GenAI error] GOOGLE_API_KEY is not set. "
            "Set env var or pass api_key to generate_playbook()."
        )
